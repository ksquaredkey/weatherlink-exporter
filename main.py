...
weatherlink_url = os.getenv("WEATHERLINK_URL", default="http://weatherlink.ksquared.net/v1/current_conditions")
# weatherlink_url = "http://weatherlink.ksquared.net/v1/current_conditions"

poll_interval = int(os.getenv("WEATHERLINK_POLL_INTERVAL", default=3))
# poll_interval = 3
...
if __name__ == '__main__':
    ...
    while True:
        # Get the current conditions from the WeatherLink 6100
        r = requests.get(weatherlink_url)
        j = r.json()
        data = j['data']

        # Set the global device ID and timestamp of measurement.
        did.info({'device': data['did']})
        ts.set(data['ts'])

        # Each Sensor reports as a separate JSON object in the list of conditions
        for conds in data['conditions']:
            t = conds['data_structure_type']
            try:
                # look up sensor by type.  Order in list is not guaranteed.
